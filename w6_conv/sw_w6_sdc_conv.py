...
# I/O
dirname = "williamson_6_sdc_ref%s_dt%s_k%s_deg%s" % (ref_level, dt_true, 1, degree)
...
for dt in dts:
    ...
    for s in scheme_index:
        ...
        # I/O
        dirname = "williamson_6_sdc_ref%s_dt%s_k%s_deg%s" % (ref_level, dt, s, degree)
        ...
        if (s==1):
           ...
           Mu=1
           ku=1
           base_u=ForwardEuler(domain,"u")
           base_D=ForwardEuler(domain,"D")
           transported_fields = [FE_SDC(base_u,domain,Mu,ku, quadrature="gauss-legendre", field_name="u", final_update=True),
                                 FE_SDC(base_D,domain, Md, kd, quadrature="gauss-legendre",field_name="D",final_update=True)]
        elif(s==2):
           Md=2
           kd=2
           Mu=2
           ...
           base_u=ForwardEuler(domain,"u")
           base_D=ForwardEuler(domain,"D")
           transported_fields=[FE_SDC(base_u,domain,M,k, quadrature="gauss-legendre",field_name="u",final_update=True),
                               FE_SDC(base_D,domain,M,k, quadrature="gauss-legendre",field_name="D",final_update=True)]
